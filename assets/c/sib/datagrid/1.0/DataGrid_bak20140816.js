...
define(function(require, exports, module) {
     ...
     */
    var _fnScrollBarWidth = (function(){
        var _scrollBarWidth = 0;//保存滚动条宽度,只计算一次

        return function _fnScrollBarWidth () {
        ...
        private : {
            ...
            _setSize : function () {
                ...
                if(hasXscroll && !hasYscroll){
                    //alert('X方向有滚动条');
                    if(opts.height != 'auto' && ($view2Table.outerHeight() + _fnScrollBarWidth() > $view2body.height())){
                        hasYscroll = true;
                    }
                } else if(hasYscroll && !hasXscroll){
                    //alert('Y方向有滚动条');
                    if($view2Table.outerWidth() + _fnScrollBarWidth() > $view2body.width()){
                        hasXscroll = true;
                    }
                }
                ...
                if(hasYscroll && hasXscroll){ //x,y都有滚动条
                    ...
                    $view1body.find('>table').css('margin-bottom', _fnScrollBarWidth());
                    ...
                    if(opts.height == 'auto') { //fixed IE67 BUG !!! 其他浏览器高度也要加个滚动条的宽度
                        $view2body.height($view2body.height() + _fnScrollBarWidth());
                        $grid.find('.a-datagrid-view').height($grid.find('.a-datagrid-view').height() + _fnScrollBarWidth());
                    }
                } else if(hasYscroll && !hasXscroll){   //仅Y有滚动条
                    $view2body.find('>table').outerWidth($view2body.width() - _fnScrollBarWidth());
                } else {    //X,Y 都没有
                ...
                if(hasXscroll){
                    $view2header.find('.a-datagrid-header-inner').outerWidth(view2bodyTableW + _fnScrollBarWidth());
                }
            ...
            _bindEvents : function () {
                ...
                if(opts.width == 'auto' || opts.width == '100%') {
                    $grid.resize((function(e){
                        ...
                        return SIB.buffer(function() { //提高效率
            ...
            _buildBackboard : function (){
                ...
                for(var i = 0; i < mixColumns.length; i++) {
                    ...
                    var label = '<label><input type="checkbox" {isHidden} name="{fieldName}" /><span>{colTitle}</span></label>';
                    var isHidden = '';
                    var colTitle = '';
        ...
        public : {
            ...
            _loadData : function (){ //加载数据
                ...
                if (opts.url) {
                    ...
                    if(opts.pagination) {
                        var total = (opts.data.rows && opts.data.rows.length) || opts.data.total || 0;
                        var pn = opts.pageNumber || 1;
                        var start = (pn - 1) * opts.pageSize;
                        var end   = (start + opts.pageSize > total ) ? total : start + opts.pageSize;
                        var data = {total : total, rows : []};
                        if($.isArray(opts.data.rows)) {
                            data.rows = opts.data.rows.slice(start, end);
                        }
                        this._showData(data);
                    } else {
            ...
            _showData : function (data){
                ...
                viewPlugin.render.call(viewPlugin, self, $view2body.find('table'), false );
                if (opts.rownumbers || (uniqueFrozenCols && uniqueFrozenCols.length > 0)){
                    viewPlugin.render.call(viewPlugin, self, $view1body.find('table'), true );
                }
    ...
    //Grid默认构造html代码
    D.plugins['default'] = {
        render : function(oGrid, $container, forzen) {
